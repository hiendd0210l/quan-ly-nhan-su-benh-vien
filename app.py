...
# 2. CHUỖI BASE64 AVATAR ADMIN (ĐOÀN DANH HIỂN)
ADMIN_AVATAR = "https://i.imgur.com/8Q9eZ3X.jpg"  # Đã tối ưu hiển thị avatar cá nhân
...
# 4. KẾT NỐI CSDL POSTGRESQL (NEON)
@st.cache_resource
def init_connection():
    try:
...
# 5. CHUỖI BASE64 LOGO CHUẨN BỆNH VIỆN BƯU ĐIỆN
...
def login_screen():
    st.markdown("""
        <style>
            ...
            .hospital-logo-img { width: 140px; height: 140px; object-fit: contain; margin-bottom: 10px; filter: drop-shadow(0px 4px 8px rgba(0, 0, 0, 0.12)); }
            .login-title { text-align: center; color: #0056b3; font-size: 24px; font-weight: 800; margin-bottom: 4px; text-transform: uppercase; }
            .login-subtitle { text-align: center; color: #64748b; font-size: 14px; margin-bottom: 20px; font-weight: 500; }
            [data-testid="stForm"] { background: linear-gradient(145deg, #ffffff, #f8fafc); padding: 30px 25px; border-radius: 16px; box-shadow: 0 10px 25px rgba(13, 71, 161, 0.08); border: 1px solid #e2e8f0; }
            div[data-testid="stFormSubmitButton"] > button { background: linear-gradient(135deg, #0284c7, #0369a1) !important; color: white !important; font-weight: 700 !important; border: none !important; border-radius: 8px !important; padding: 10px 0px !important; }
        </style>
...
st.sidebar.markdown(
    ...
    <div style="text-align: center; padding-bottom: 10px;">
        ...
        <p style="font-size: 13px; color: #a0aec0;">👤 Xin chào: <b>{st.session_state.user_info.get('fullname', st.session_state.user_info['name'])}</b></p>
    </div>
    """,
    unsafe_allow_html=True
)
